...
router.put('/:programId/instructors/:instructorId', async (req, res, next) => {
	try {
		...
		const result = await updateProgramInstructor(req.params.programId, req.params.instructorId, req);
		res.status(201).send(result);
	} catch (e) {
		next(e);
	}
...
router.delete('/:programId/instructors/:instructorId', async (req, res, next) => {
	try {
		await paramsValidator.validateAsync({
			programId: req.params.programId,
			instructorId: req.params.instructorId,
		});
		const result = await deleteProgramInstructor(req.params.programId, req.params.instructorId);
		if (result) {
			res.status(200).send({
...
router.put('/:programId/lessons/:lessonId', async (req, res, next) => {
	try {
		...
		const result = await updateProgramLesson(req.params.programId, req.params.lessonId, req);
		res.status(201).send(result);
	} catch (e) {
		next(e);
	}
...
router.delete('/:programId/lessons/:lessonId', async (req, res, next) => {
	try {
		...
		const result = await deleteProgramLesson(req.params.programId, req.params.lessonId);
		if (result) {
...
router.post('/:programId/lessons/:lessonId/concepts', async (req, res, next) => {
	try {
		...
		const result = await postLessonConcept(req.params.programId, req.params.lessonId, req);
...
router.put('/:programId/lessons/:lessonId/concepts/:conceptId', async (req, res, next) => {
	try {
		await conceptValidator.validateAsync({
			title: req.body.title,
			url: req.body.url,
		...
		const result = await updateLessonConcept(req.params.programId, req.params.lessonId, req.params.conceptId, req);
		res.status(201).send(result);
	} catch (e) {
		next(e);
	}
...
router.delete('/:programId/lessons/:lessonId/concepts/:conceptId', async (req, res, next) => {
	try {
		await paramsValidator.validateAsync({
			programId: req.params.programId,
			lessonId: req.params.lessonId,
		...
		const result = await deleteLessonConcept(req.params.programId, req.params.lessonId, req.params.conceptId);
		if (result) {
			res.status(200).send({
				message: `Lesson with Id ${result} is deleted successfully`,
			});
		} else {
			res.status(404).send({
				message: `No concept found with Id ${req.params.conceptId}`,
			});
		}
	} catch (e) {
		next(e);
